...
export default function Appointment(props){
    ...
    return (
        <div className={`row py-5 underline ${props.item.deletd ? ' bc-red' : props.item.done ? ' bc-green' : ''}`} key={props.item.id}>
            ...
            <div className={`column importance ${props.item.levelOfImportance === 0 ? ' bc-green' : props.item.levelOfImportance === 4 ? ' bc-gold' : props.item.levelOfImportance === 5 ? ' bc-red' : ''}`}>
                {levelOfImportance[props.item.leveOfImportance]}</div>
